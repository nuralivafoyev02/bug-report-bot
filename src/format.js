...
function buildReportMessage(report, timezone) {
  ...
  if (report.pm_name) lines.push(`<b>PM:</b> ${escapeHtml(report.pm_name)}`);
  if (report.assignee_name) lines.push(`<b>Assigned to:</b> ${escapeHtml(report.assignee_name)}`);
  if (report.last_action) lines.push(`<b>Last action:</b> ${escapeHtml(report.last_action)}`);
...
function buildMainKeyboard(reportCode, currentPriority) {
  return {
    inline_keyboard: [
      [
        { text: '✅ Qabul qilindi', callback_data: `sr|ac|${reportCode}` },
        { text: '👤 PM oldi', callback_data: `sr|pm|${reportCode}` }
      ],
      [
        { text: '🧑‍💻 Assign', callback_data: `sr|as|${reportCode}` },
        { text: '⏳ Jarayonda', callback_data: `sr|ip|${reportCode}` }
      ],
      [
        { text: '💬 Savol bor', callback_data: `sr|ni|${reportCode}` },
        { text: '✅ Yopildi', callback_data: `sr|rs|${reportCode}` }
      ],
      [
        { text: `${currentPriority === 'urgent' ? '• ' : ''}🔴 Urgent`, callback_data: `sr|p|${reportCode}|urgent` },
        { text: `${currentPriority === 'high' ? '• ' : ''}🟠 High`, callback_data: `sr|p|${reportCode}|high` }
      ],
      [
        { text: `${currentPriority === 'medium' ? '• ' : ''}🟡 Medium`, callback_data: `sr|p|${reportCode}|medium` },
        { text: `${currentPriority === 'low' ? '• ' : ''}🟢 Low`, callback_data: `sr|p|${reportCode}|low` }
      ],
      [
        { text: '❌ Bekor', callback_data: `sr|cx|${reportCode}` }
      ]
    ]
  };
}
...
function buildAssignKeyboard(reportCode, assignees) {
  const rows = assignees.map((assignee) => [
    { text: `👤 ${assignee.label}`, callback_data: `sr|g|${reportCode}|${assignee.id}` }
  ]);
  rows.push([{ text: '⬅️ Ortga', callback_data: `sr|bk|${reportCode}` }]);
